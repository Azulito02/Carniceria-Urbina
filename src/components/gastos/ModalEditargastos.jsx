...
import './ModalEditarGastos.css';

const ModalEditarGastos = ({ isOpen, onClose, onSave, gasto, loading }) => {
  ...
  useEffect(() => {
    if (isOpen && gasto) {
      ...
      setMonto(gasto.monto ? String(gasto.monto) : '');
      setError('');
    }
  }, [isOpen, gasto]);
  ...
  const handleSubmit = (e) => {
    ...
    onSave(gasto.id, { descripcion: descripcion.trim(), monto: montoNum });
  };
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="modal-footer">
            ...
            <button type="submit" className="btn-guardar" disabled={loading}>
              {loading ? <i className="fas fa-spinner fa-spin"></i> : 'Actualizar'}
            </button>
          </div>
        </form>
      </div>
