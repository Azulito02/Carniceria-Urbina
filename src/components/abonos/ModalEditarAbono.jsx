...
const ModalEditarAbono = ({ isOpen, onClose, onSave, abono, creditos, loading }) => {
  ...
  // Precargar datos del abono al abrir el modal
  useEffect(() => {
    if (abono) {
      setFormData({
        ...
        banco: abono.banco || '',
        fecha: abono.fecha ? new Date(abono.fecha).toISOString().slice(0, 16) : '',
        observaciones: abono.observaciones || ''
      });
    }
  }, [abono]);

  // Actualizar saldo pendiente al cambiar el crédito
  useEffect(() => {
  ...
  const handleSubmit = (e) => {
    ...
    setErrorLocal('');
    onSave(abono.id, { ...formData, saldo_pendiente: saldoPendiente });
  };
